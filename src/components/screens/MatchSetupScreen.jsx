...
export default function MatchSetupScreen() {
  ...
  return (
    <div className="pb-[120px] bg-[#F7F8F4] min-h-screen">
        ...
        <div className="flex items-center justify-between max-w-sm mx-auto">
          {steps.map((s, idx) => (
            <div key={s.num} className="flex-1 text-center relative">
               <div className={`text-[11px] font-bold uppercase tracking-wider mb-2 ${s.num === currentStep ? 'text-[#2457D6]' : s.num < currentStep ? 'text-[#0FA968]' : 'text-[#8a99b0]'}`}>
                 {s.label}
               </div>
      ...
      <div className="p-4 space-y-6">
        ...
        {currentStep === 1 && (
          <div className="bg-white rounded-[20px] p-5 shadow-sm border border-gray-100 animate-slide-up">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-[11px] font-bold text-[#8a99b0] uppercase tracking-wider mb-1.5">Team A Name *</label>
                <input 
                  type="text" value={matchSetup.teamA} onChange={(e) => setMatchSetup({ ...matchSetup, teamA: e.target.value })}
                  className="w-full bg-[#F7F8F4] rounded-[12px] p-4 text-[16px] font-black outline-none border focus:border-[#2457D6] border-transparent"
                />
              </div>
              <div className="text-center font-black text-[12px] text-[#8a99b0] uppercase tracking-widest">VS</div>
              <div>
                <label className="block text-[11px] font-bold text-[#8a99b0] uppercase tracking-wider mb-1.5">Team B Name *</label>
                <input 
        ...
        {currentStep === 2 && (
          <div className="space-y-4 animate-slide-up">
            <div className="bg-white rounded-[20px] p-5 shadow-sm border border-gray-100">
              ...
              <label className="block text-[11px] font-bold text-[#8a99b0] uppercase tracking-wider mb-2">Toss Winner</label>
            ...
            <div className="bg-white rounded-[20px] p-5 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setIsEditingXI(!isEditingXI)} className="text-[11px] font-bold uppercase tracking-wider text-[#2457D6] flex items-center gap-1">
              ...
              <div className="space-y-2">
                {filteredPlayers.map(p => (
                  <div key={p.id} className="flex items-center justify-between bg-[#F7F8F4] p-3 rounded-[12px]">
                    ...
                    {isEditingXI && (
                      <div className="flex gap-1.5">
                         <button onClick={() => toggleRole(p.id, 'captain')} className="w-8 h-8 rounded-full bg-white border border-gray-200 text-[#101827] text-[10px] font-bold shadow-sm">C</button>
                         <button onClick={() => toggleRole(p.id, 'wk')} className="w-8 h-8 rounded-full bg-white border border-gray-200 text-[#101827] text-[10px] font-bold shadow-sm">WK</button>
        ...
        {currentStep === 3 && (
          <div className="bg-white rounded-[20px] p-5 shadow-sm border border-gray-100 animate-slide-up">
             ...
             <div className="space-y-4">
                <div>
                  <label className="block text-[11px] font-bold text-[#8a99b0] uppercase tracking-wider mb-1.5">Total Overs</label>
                  <select value={matchSetup.totalOvers} onChange={e => setMatchSetup(p => ({ ...p, totalOvers: Number(e.target.value) }))} className="w-full bg-[#F7F8F4] rounded-[12px] p-3 text-[14px] font-bold outline-none border border-transparent appearance-none">
                ...
                <div>
                  <label className="block text-[11px] font-bold text-[#8a99b0] uppercase tracking-wider mb-1.5">Wide Penalty</label>
                  <select value={matchSetup.widePenalty} onChange={e => setMatchSetup(p => ({ ...p, widePenalty: Number(e.target.value) }))} className="w-full bg-[#F7F8F4] rounded-[12px] p-3 text-[14px] font-bold outline-none border border-transparent appearance-none">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-100 shadow-[0_-10px_20px_rgba(0,0,0,0.02)] z-30">
        <div className="flex gap-3">
          {currentStep > 1 && (
            <button onClick={() => setCurrentStep(c => c - 1)} className="flex-1 py-4 rounded-[16px] bg-[#F7F8F4] text-[#101827] text-[14px] font-bold border border-gray-200 active:bg-gray-100">
              Back
            </button>
          )}
          <button 
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-[#101827]/65 p-3 sm:p-5">
           <div className="w-full max-w-sm bg-white rounded-[24px] shadow-2xl overflow-hidden animate-slide-up">
              <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                <h3 className="text-[16px] font-black text-[#101827]">Roster</h3>
              ...
              <div className="p-4">
                 <input type="text" placeholder="Search players..." value={rosterSearchQuery} onChange={e => setRosterSearchQuery(e.target.value)} className="w-full bg-[#F7F8F4] rounded-[12px] p-3 text-[14px] outline-none mb-4" />
                    ...
                    {players.filter(p => !matchSetup.playingXI.find(xi => xi.id === p.id) && p.name.toLowerCase().includes(rosterSearchQuery.toLowerCase())).map(p => (
                       <button key={p.id} onClick={() => { addPlayerToXI(p); setIsAddPlayerModalOpen(false); }} className="w-full flex items-center justify-between p-3 bg-white border border-gray-200 rounded-[12px] active:bg-gray-50">
                          <div className="text-left">
                            ...
                            <div className="text-[11px] text-[#8a99b0]">{p.role}</div>
                          </div>
